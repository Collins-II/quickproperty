...
interface BookingSlotProps {
  onBookingChange: (startDate: Date | undefined,
    endDate: Date | undefined, duration: number, totalCharge: number) => void;
}

const BookingSlot: React.FC<BookingSlotProps> = ({ onBookingChange }) => {
  ...
  const handleRangeChange = useCallback((item: RangeKeyDict) => {
    ...
    if (item.selection.endDate && item.selection.startDate) {
      ...
      onBookingChange(item?.selection?.startDate, item.selection.endDate, duration * 24 * 60, totalCharge);
    }
  }, [onBookingChange]);
